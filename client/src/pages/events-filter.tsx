...
const sections: FeatureSection[] = [
  {
    ...
    content: (
      <ul>
        ...
        <li>Directly tied to Task List #2 “Filter by Sport”.</li>
      </ul>
    ),
  },
  {
    ...
    content: (
      <ul>
        <li>Primary dropdown listing supported sports (basketball, soccer, tennis, etc.).</li>
  ...
  },
  {
    ...
    content: (
      <ul>
        <li>
          Use <code>{'api.events.getEvents({ sport: selectedSport })'}</code> which wraps
          <code>{"eq('sport', selectedSport)"}</code>.
        </li>
        <li>When “All” is chosen, skip the filter and return cached events.</li>
        <li>Consider local caching via React Query to avoid duplicate round-trips.</li>
      </ul>
    ),
  },
  {
